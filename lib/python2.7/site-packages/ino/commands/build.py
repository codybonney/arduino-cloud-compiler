...
class Build(Command):
    ...
    *.[c|cpp|pde|ino] files. They are compiled and linked into resulting
    ...
    def make(self, makefile, **kwargs):
        ...
        ret = Popen([self.e.make, '-f', makefile, 'all'], stderr=PIPE)
        ...
        #ret = subprocess.Popen([self.e.make, '-f', makefile, 'all'], stdout=subprocess.PIPE).communicate()[0]
        #print ret
        #if ret != 0:
        #    raise Abort("Make failed with code %s" % ret)
        ...
        #try:
        #    subprocess.check_output([self.e.make, '-f', makefile, 'all'], stderr=subprocess.STDOUT)
